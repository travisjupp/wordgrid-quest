...
import tempMaterialReducer, { setCategory, addItem, removeItem, resetTempMaterial } from '@features/tempMaterial/tempMaterialSlice';

describe('tempMaterialSlice', () => {
  it('should set the category when setCategory is dispatched', () => {
    const initialState: InitialTempMaterialSliceState = { category: '' , discoveryTerms: [] };
  ...
  it('should add item when addItem is dispatched', () => {
    ...
    expect(nextState.discoveryTerms)
      .toEqual([{dt: 'Whale', def: 'Largest mammals on Earth'}]);
  });

  it('should remove item when removeItem is dispatched', () => {
    const initialState: InitialTempMaterialSliceState = { category: 'Mammals' , 
